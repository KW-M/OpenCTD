...
#define LED_STAT1 LED_BUILTIN  // Indicator light 1 (Red, near plug, Soldered onto feather board).
#define LED_STAT2 12           // bright white LED on PCB.
#define LED_STAT3 8            // green led on feather board
...
like the light_sensor, or components embeded in the ctd tube, like the temperature probes. They are seperate to allow saved calibration of componenents
...
typedef struct
{
  ...
  float seconds_between_log_events = 1.0;  // default value is 1 log row per second (while you can go lower like 0.25, the e conductivity and temp sensors will still only update at their maximum frequecy of ~1 second).
